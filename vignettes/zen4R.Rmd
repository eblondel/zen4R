...
myrec$setSubjects(c("R","package","software"))
...
For a record already published (with a DOI), it is very straightforward to submit it to communities (whether or not you are the manager of these communities). To do so, run the following code over your `record`:
...
In case a community name doesn't match an existing community, some warning will be raised without any blocking error.
...
The new Zenodo platform makes it possible to add a draft record to a community. The process is equivalent to what was possible with the old version of Zenodo and zen4R < 0.10, which was achieved using the metadata elements. With the new Zenodo, adding a draft record is part of a record review process and is carried out using 2 different methods:

* a first method creates a review request by "attaching" the draft record to a community:
...
* a second method *submits* the review request to the community manager:

```r
zenodo$submitRecordForReview(record$id)
```

Indeed, adding a draft deposit/record to a community now means that you are requesting the record to be *reviewed* for publication in a community. The request for addition to the community similar that of a published record (as above), but the manager must first review the record. Consequently:
